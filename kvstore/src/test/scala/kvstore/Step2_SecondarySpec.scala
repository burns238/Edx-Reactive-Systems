...
trait Step2_SecondarySpec
  ...
  test("Step2-case1: Secondary (in isolation) should properly register itself to the provided Arbiter") {
    val arbiter = TestProbe()
        val secondary = system.actorOf(Replica.props(arbiter.ref, Persistence.props(flaky = true)), "step2-case1-secondary")
  ...
  test("Step2-case2: Secondary (in isolation) must handle Snapshots") {
    ...
    val replicator = TestProbe()
        val secondary = system.actorOf(Replica.props(arbiter.ref, Persistence.props(flaky = true)), "step2-case2-secondary")
  ...
  test("Step2-case3: Secondary should drop and immediately ack snapshots with older sequence numbers") {
    ...
    val arbiter = TestProbe()
    val replicator = TestProbe()
        val secondary = system.actorOf(Replica.props(arbiter.ref, Persistence.props(flaky = true)), "step2-case3-secondary")
        val client = session(secondary)
  ...
  test("Step2-case4: Secondary should drop snapshots with future sequence numbers") {
    ...
    val arbiter = TestProbe()
    val replicator = TestProbe()
        val secondary = system.actorOf(Replica.props(arbiter.ref, Persistence.props(flaky = true)), "step2-case4-secondary")
        val client = session(secondary)
