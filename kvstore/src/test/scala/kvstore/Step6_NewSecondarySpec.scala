...
trait Step6_NewSecondarySpec
  ...
  test("Step6-case2: Primary must stop replication to removed replicas and stop Replicator") {
    val probe = TestProbe()
    val arbiter = TestProbe()
        val primary = system.actorOf(Replica.props(arbiter.ref, Persistence.props(flaky = true)), "step6-case2-primary")
        val user = session(primary)
    val secondary = TestProbe()
  ...
  test("Step6-case3: Primary must stop replication to removed replicas and waive their outstanding acknowledgements") {
    val arbiter = TestProbe()
        val primary = system.actorOf(Replica.props(arbiter.ref, Persistence.props(flaky = true)), "step6-case3-primary")
        val user = session(primary)
    val secondary = TestProbe()
